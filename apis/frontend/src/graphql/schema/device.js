...
queryResolvers.device = async (obj, { secret }, context) => {
...
mutationResolvers.claimDevice = async (obj, { secret }, context) => {
...
mutationResolvers.setDevicePin = async (obj, { secret, pin }, context) => {
	const { deviceBySecretLoader } = context
...
mutationResolvers.setDeviceName = async (obj, { secret, name }, context) => {
	const { deviceBySecretLoader } = context
	const device = await deviceBySecretLoader.load(secret)
	if (!device) {
		throw new ApolloError(error.DEVICE_DOES_NOT_EXIST)
